...
public class CreateItemCommand : IRequest<CreatedItemResponse>, ISecuredRequest, ICacheRemoverRequest, ILoggableRequest, ITransactionalRequest
{
    ...
    public string[] Roles => [Admin, Write, ItemsOperationClaims.Create];
    ...
    public class CreateItemCommandHandler : IRequestHandler<CreateItemCommand, CreatedItemResponse>
    {
        ...
        public async Task<CreatedItemResponse> Handle(CreateItemCommand request, CancellationToken cancellationToken)
        {
            ...
            CreatedItemResponse response = _mapper.Map<CreatedItemResponse>(item);
            return response;
        }
    }
}